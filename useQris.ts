...
export function useQrisPoster() {
    ...
    return { buildQrisPosterDataUrl, buildQrisPosterBlob, buildQrisPosterCanvas, downloadQrisPoster, parseQrisTLV, toDynamicQris };
}
...
export type ConvenienceFee = { type: 'flat'; value: number | string } | { type: 'percent'; value: number | string };

export interface ToDynamicOptions {
...
function scanTopLevel(str: string): TLVItem[] {
    const out: TLVItem[] = [];
    let i = 0;
    const n = str.length;
    while (i + 4 <= n) {
        ...
        const value = str.slice(valStart, valEnd);
...
function encodeTopLevel(items: TLVItem[], includeCRC = false): string {
    const parts: string[] = [];
    for (const it of items) {
        if (!includeCRC && it.tag === '63') continue;
        const lenStr = it.len.toString().padStart(2, '0');
        parts.push(it.tag + lenStr + it.value);
    }
    return parts.join('');
}

function findIndexByTag(items: TLVItem[], tag: string): number {
    return items.findIndex((i) => i.tag === tag);
}

function insertBeforeTag(target: string, items: TLVItem[], inserts: TLVItem[]): TLVItem[] {
    const idx = findIndexByTag(items, target);
    if (idx === -1) return [...items, ...inserts];
    const head = items.slice(0, idx);
    const tail = items.slice(idx);
    return [...head, ...inserts, ...tail];
}

function formatAmountForTag54(amount: number | string, currency: string): string {
    const n = typeof amount === 'string' ? Number(amount) : amount;
    if (!Number.isFinite(n)) return String(amount).replace(/[^\d.]/g, '');

    if (currency === '360') return Math.round(n).toString();

    const fixed = Number.isInteger(n) ? n.toString() : n.toFixed(2);
    return fixed.replace(/\.?0+$/, '');
}
function crc16CCITT(bytes: Uint8Array): number {
    let crc = 0xffff; // CCITT-FALSE init
    for (let i = 0; i < bytes.length; i++) {
        crc ^= bytes[i] << 8;
        for (let j = 0; j < 8; j++) {
...
function validateQrisCrc(str: string): boolean {
    ...
    const ibm = crc16IBM(asciiToBytes(data)).toString(16).toUpperCase().padStart(4, '0');
    const ccitt = crc16CCITT(asciiToBytes(data)).toString(16).toUpperCase().padStart(4, '0');
    return crcGiven === ibm || crcGiven === ccitt;
}

function computeQrisCrc(strUpTo6304: string): string {
    return crc16CCITT(asciiToBytes(strUpTo6304)).toString(16).toUpperCase().padStart(4, '0');
}
export function toDynamicQris(raw: string, options: ToDynamicOptions): string {
    const input = (raw || '').trim();
    if (!input) throw new Error('Empty QRIS payload.');

    const idx6304 = input.indexOf('6304');
    const base = idx6304 >= 0 ? input.slice(0, idx6304 + 4) : input + '6304';

    const items = scanTopLevel(base);

    const idx01 = findIndexByTag(items, '01');
    if (idx01 >= 0) {
        items[idx01].value = '12';
        items[idx01].len = 2;
    } else {
        items.unshift({ tag: '01', len: 2, value: '12' });
    }

    const tag53 = items.find((it) => it.tag === '53');
    const currency = (options.currency || (tag53?.value ?? '360')).trim();
    if (tag53) {
        tag53.value = currency;
        tag53.len = currency.length;
    } else {
        const curItem: TLVItem = { tag: '53', len: currency.length, value: currency };
        items.splice(1, 0, curItem);
    }

    for (const t of ['54', '55', '56', '57']) {
        const i = findIndexByTag(items, t);
        if (i >= 0) items.splice(i, 1);
    }

    const amountStr = formatAmountForTag54(options.amount, currency);
    const tag54: TLVItem = { tag: '54', len: amountStr.length, value: amountStr };

    const inserts: TLVItem[] = [tag54];

    if (options.convenienceFee) {
        const fee = options.convenienceFee;
        if (fee.type === 'flat') {
            const ind: TLVItem = { tag: '55', len: 2, value: '02' };
            const feeVal = formatAmountForTag54(fee.value, currency);
            const tag56: TLVItem = { tag: '56', len: feeVal.length, value: feeVal };
            inserts.push(ind, tag56);
        } else if (fee.type === 'percent') {
            const ind: TLVItem = { tag: '55', len: 2, value: '03' };
            // keep up to 2 decimals for percent, e.g. "1.5" => "1.5"
            const pctRaw = typeof fee.value === 'string' ? fee.value : fee.value.toString();
            const pct = pctRaw.replace(/[^\d.]/g, '').replace(/^0+(\d)/, '$1');
            const tag57: TLVItem = { tag: '57', len: pct.length, value: pct };
            inserts.push(ind, tag57);
        }
    }

    const withAmount = insertBeforeTag('58', items, inserts);

    const withoutCRC = encodeTopLevel(withAmount, false) + '6304';
    const crc = computeQrisCrc(withoutCRC);
